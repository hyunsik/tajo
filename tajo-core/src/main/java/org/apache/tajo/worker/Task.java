...
public class Task {
  ...
  private FileChunk getLocalStoredFileChunk(URI fetchURI, TajoConf conf) throws IOException {
    ...
    if (shuffleType.equals("r")) {
      ...
      Path path = executionBlockContext.getLocalFS().makeQualified(executionBlockContext.getLocalDirAllocator()
          .getLocalPathToRead(queryBaseDir + ta + "/output/", conf));
      ...
      Path path = executionBlockContext.getLocalFS().makeQualified(executionBlockContext.getLocalDirAllocator()
          .getLocalPathToRead(partPath, conf));
